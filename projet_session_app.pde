...
void setup() {
  ...
  if(cameras.length == 0) {
    println("Aucune camera disponible pour la capture");
    exit();
  }
  else {
    println("Cameras disponible:");
    camWidth = 9999;
    camHeight = 9999;
    for (int i = 0; i < cameras.length; i++) {
      // Trouver le debut du width
      int w = cameras[i].indexOf("size=");
      // Trouver fin du width et debut du height 
      int h = cameras[i].indexOf("x");
      // Trouver fin du height
      int f = cameras[i].indexOf(",fps");
      
      int tempcamwidth = Integer.parseInt(cameras[i].substring(w+5, h));
      if(tempcamwidth <= camWidth) {
        int tempcamheight = Integer.parseInt(cameras[i].substring(h+1, f));
        if(tempcamheight <= camHeight) {
          camWidth = tempcamwidth;
          camHeight = tempcamheight;
        }
      }
      println(cameras[i]);
    }
  ...
  fullScreen();
  //surface.setSize(camWidth * 3, camHeight * 2);
  textSize(24);
        ...
        for(int i = 0; i < 50; i++){
          ...
          for(int j = 0; j < users.length; j++){
            if(!users[j].equals(trainText)){
              ...
              /*loadData(theFace);
              resultActualFace = neuralNet.respond(testing_set.inputs);
              for(int y = 0; y < resultActualFace.length; y++){
                if((resultActualFace[y] > temp[y] - 0.25)&&(resultActualFace[y] < temp[y] + 0.25)){
                  if(resultActualFace[y] < temp[y]){
                    if(temp[y] - 0.25 > -1 )
                      resultActualFace[y] = temp[y] - 0.25;
                    else
                      resultActualFace[y] = -0.999999;
                  }else {
                    if(temp[y] + 0.25 > 1 )
                      resultActualFace[y] = temp[y] + 0.25;
                    else
...
void getFaceForRecognition(){
  ...
  for(int i = 0; i < 256; i++){
    line(camWidth + 100 + i,(height * 0.60) - thePixels[i]/4, camWidth + 100 + i,(height * 0.60));
  }
  f.resize(25,25);
  theFace = f.copy();
}
